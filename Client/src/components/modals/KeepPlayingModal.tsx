...
export default function KeepPlayingModal({
  ...
  const title = popupConfig?.value?.title
    ? decodeHtmlEntities(popupConfig.value.title)
    : "Time's Up!";
  const subtitle = popupConfig?.value?.subtitle
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50">
      <div className="relative bg-white dark:bg-[#475568] rounded-2xl shadow-xl p-4 sm:p-6 md:p-8 w-[90%] sm:w-[450px] md:w-[500px] lg:w-[550px] max-w-[90vw] border-4 border-[#C026D3]">
        <div className="flex flex-col items-center max-w-[90%] mx-auto">
          <h1 className="text-lg sm:text-xl lg:text-xl tracking-wide font-extrabold mb-3 sm:mb-4 text-[#18181b] dark:text-white text-center">
            {title}
          </h1>
          <p className="text-base sm:text-md lg:text-lg text-center mb-4 sm:mb-6 text-gray-600 dark:text-gray-300">
            {subtitle}
          </p>
